  ...
  return (
    <>
      <section className="relative min-h-[58vh] w-full overflow-hidden bg-[#07090c] md:hidden">
        ...
        <div className="absolute inset-0 bg-[#06080b]/28" />
        <div className="absolute inset-0 bg-gradient-to-r from-black/62 via-black/26 to-black/30" />
          ...
          <div className="mt-1 flex items-end gap-1.5">
            {countdownItems.map((item, index) => (
              <div key={`mobile-${item.label}`} className="flex items-end gap-1.5">
                <span className={`${anton.className} text-[24px] leading-none text-white`}>
                  {item.value}
                </span>
                {index !== countdownItems.length - 1 && (
                  <span className={`${anton.className} text-[20px] leading-none text-white/80`}>
                    :
                  </span>
                )}
              </div>
        ...
        <div className="relative z-10 mx-auto flex min-h-[58vh] w-full max-w-[1600px] flex-col justify-end px-4 pb-5 pt-8">
          ...
          <Button
            variant="accent"
            className="mt-5 w-fit cursor-pointer rounded-none px-7 py-3 text-sm font-semibold uppercase tracking-[0.14em]"
          >
            Shop The Drop
      ...
      <section className="relative hidden min-h-[56vh] w-full overflow-hidden bg-[#07090c] md:block lg:min-h-[50vh]">
        ...
        <div className="absolute inset-0 bg-[#06080b]/20" />
        <div className="absolute inset-0 bg-gradient-to-r from-black/55 via-black/18 to-black/35" />
        ...
        <div className="relative z-10 mx-auto flex min-h-[56vh] w-full max-w-[1600px] flex-col justify-between px-6 py-8 md:px-8 md:py-10 lg:min-h-[50vh] lg:px-10 lg:py-14 xl:px-14">
          <div>
            ...
            <Button
              variant="accent"
              className="mt-6 cursor-pointer rounded-none px-6 py-3 text-sm font-semibold uppercase tracking-[0.14em] md:mt-7 md:px-8 md:py-3.5 md:text-sm lg:mt-10 lg:px-10 lg:py-4 lg:text-base lg:tracking-[0.18em]"
            >
              Shop The Drop
              <ArrowRight className="h-4 w-4" strokeWidth={2.1} />
            </Button>
