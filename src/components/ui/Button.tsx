...
export default function Button({
  ...
  return (
    <button
      type={type}
      className={`${inter.className} inline-flex cursor-pointer items-center justify-center gap-2 rounded-none px-6 py-2.5 text-sm transition-colors disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50 ${variantStyles[variant]} ${className}`}
      {...props}
    >
      {children}
    </button>
